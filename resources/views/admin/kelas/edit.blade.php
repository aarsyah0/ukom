                                    ...
                                    <select class="form-select @error('mahasiswa_id') is-invalid @enderror" id="mahasiswa_id" name="mahasiswa_id" required>
                                        ...
                                        @foreach($mahasiswa as $mhs)
                                            <option value="{{ $mhs->id }}"
                                                ...
                                                {{ old('mahasiswa_id', $kelas->mahasiswa_id) == $mhs->id ? 'selected' : '' }}>
                                    ...
                                    <select class="form-select @error('jadwal_id') is-invalid @enderror" id="jadwal_id" name="jadwal_id" required>
                                        ...
                                        @foreach($jadwal as $jdl)
                                            <option value="{{ $jdl->id }}"
                                                ...
                                                {{ old('jadwal_id', $kelas->jadwal_id) == $jdl->id ? 'selected' : '' }}>
                                ...
                                <div class="alert alert-info">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <span class="text-muted">{{ $kelas->mahasiswa->nama }} ({{ $kelas->mahasiswa->nim }})</span><br>
                                            <small class="text-muted">{{ $kelas->mahasiswa->prodi }} - Semester {{ $kelas->mahasiswa->semester }}</small>
                                        </div>
                                        <div class="col-md-6">
                                            <strong>Jadwal:</strong><br>
                                            <span class="text-muted">{{ $kelas->jadwal->matakuliah->nama }}</span><br>
                                            <small class="text-muted">{{ $kelas->jadwal->dosen->nama }} | {{ $kelas->jadwal->hari }}, {{ \Carbon\Carbon::parse($kelas->jadwal->jam_mulai)->format('H:i') }} - {{ \Carbon\Carbon::parse($kelas->jadwal->jam_selesai)->format('H:i') }}</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
