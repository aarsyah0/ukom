        ...
        <div class="card-body">
            <form action="{{ route('admin.jadwal.update', $jadwal->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('ruangan') is-invalid @enderror" id="ruangan" name="ruangan" value="{{ old('ruangan', $jadwal->ruangan) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('semester') is-invalid @enderror" id="semester" name="semester" required>
                        ...
                        <option value="1" {{ old('semester', $jadwal->semester) == '1' ? 'selected' : '' }}>Semester 1</option>
                        <option value="2" {{ old('semester', $jadwal->semester) == '2' ? 'selected' : '' }}>Semester 2</option>
                        <option value="3" {{ old('semester', $jadwal->semester) == '3' ? 'selected' : '' }}>Semester 3</option>
                        <option value="4" {{ old('semester', $jadwal->semester) == '4' ? 'selected' : '' }}>Semester 4</option>
                        <option value="5" {{ old('semester', $jadwal->semester) == '5' ? 'selected' : '' }}>Semester 5</option>
                        <option value="6" {{ old('semester', $jadwal->semester) == '6' ? 'selected' : '' }}>Semester 6</option>
                        <option value="7" {{ old('semester', $jadwal->semester) == '7' ? 'selected' : '' }}>Semester 7</option>
                        <option value="8" {{ old('semester', $jadwal->semester) == '8' ? 'selected' : '' }}>Semester 8</option>
                    </select>
                    @error('semester')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="mb-3">
                    <label for="tahun_akademik" class="form-label">Tahun Akademik</label>
                    <input type="text" class="form-control @error('tahun_akademik') is-invalid @enderror" id="tahun_akademik" name="tahun_akademik" value="{{ old('tahun_akademik', $jadwal->tahun_akademik) }}" placeholder="2023/2024" required>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('jurusan') is-invalid @enderror" id="jurusan" name="jurusan" required>
                        ...
                        <option value="Teknologi Informasi" {{ old('jurusan', $jadwal->jurusan) == 'Teknologi Informasi' ? 'selected' : '' }}>Teknologi Informasi</option>
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('prodi') is-invalid @enderror" id="prodi" name="prodi" required>
                        ...
                        <option value="TIF" {{ old('prodi', $jadwal->prodi) == 'TIF' ? 'selected' : '' }}>TIF - Teknik Informatika</option>
                        <option value="TKK" {{ old('prodi', $jadwal->prodi) == 'TKK' ? 'selected' : '' }}>TKK - Teknik Komputer</option>
                        <option value="MIF" {{ old('prodi', $jadwal->prodi) == 'MIF' ? 'selected' : '' }}>MIF - Manajemen Informatika</option>
                    </select>
                    @error('prodi')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="mb-3">
                    <div class="form-check">
                        <input class="form-check-input @error('aktif') is-invalid @enderror" type="checkbox" id="aktif" name="aktif" value="1" {{ old('aktif', $jadwal->aktif) ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize prodi options if jurusan is already selected
    if (jurusanSelect.value) {
        updateProdiOptions();
        // Set the current prodi value after options are loaded
        setTimeout(() => {
            prodiSelect.value = '{{ old("prodi", $jadwal->prodi) }}';
        }, 100);
    }
});
</script>
